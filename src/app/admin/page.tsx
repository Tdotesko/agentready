...
export default function AdminPortal() {
  ...
  const [tab, setTab] = useState<"overview" | "users" | "scans" | "leads">("overview");
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const [statsRes, usersRes, leadsRes, scansRes] = await Promise.all([
    ...
    <div className="min-h-screen">
      ...
      <div className="max-w-6xl mx-auto px-6 py-8">
        ...
        <div className="flex gap-1 mb-8 bg-[var(--bg-raised)] rounded-lg p-1 w-fit">
          {(["overview", "users", "scans", "leads"] as const).map((t) => (
            <button key={t} onClick={() => setTab(t)}
              ...
              {t === "overview" ? "Overview" : t === "users" ? `Users (${users.length})` : t === "scans" ? `Scans (${scans.length})` : `Leads (${leads.length})`}
            </button>
          ))}
        </div>
